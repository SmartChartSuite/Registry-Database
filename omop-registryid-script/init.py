...
def updateConcept1(conn, row):
    ...
    sql = "UPDATE category SET section = '%s', category = '%s', question = '%s' WHERE concept_id = %s" % (row['section'],
...
def insertCategory(conn, row):
    cur = conn.cursor()
    sql = "INSERT INTO category (concept_id, section, category, question) VALUES(%s, '%s', '%s', '%s')" % (row['concept_id'], row['section'], row['category'], row['concept_name'])
    cur.execute(sql)
    conn.commit()
